...
For numerical summaries, I considered the Open Price and Close price variables and found out the mean, median, mode and IQR for both the variables. Using the group_by function, this data was obtained company wise. The table shows these values below :
...
```
Box Plot :
For box plot, I have selected the closing price variable which is plotted for each company.
```{r}
...
For scatter plot, we have plotted the Open Price and close price for a company against each other. Using the facet_wrap() function, we can see the subplots for 2 companies. 
...
```
Histogram : Here, we have plotted the Transactions of Amazon stocks.

```{r}
ggplot(data = Price_Amazon, aes(x= Transactions)) +
  geom_histogram(color = "blue" , fill = "red" ,size = 5, binwidth = 10)+
